...
my $maf=shift;#maf
my $percent=0.1;
my $length_of_scaffold_plotted=5000;
...
my %align_len;
my $link=0;
foreach my $cattle_chr(sort {$a cmp $b} keys %link){
    foreach my $cattle_start(sort {$a cmp $b} keys %{$link{$cattle_chr}}){
...
foreach my $chr(sort {$b cmp $a} keys %result){
